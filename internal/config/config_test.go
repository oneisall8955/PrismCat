...
func TestNormalizeUpstreamsDefaultsTimeout(t *testing.T) {
	...
	if got["openai"].ResponseBodyFirstByteTimeout != 0 {
		t.Fatalf("ResponseBodyFirstByteTimeout = %d, want 0", got["openai"].ResponseBodyFirstByteTimeout)
	}
	if got["openai"].ResponseBodyIdleTimeout != 0 {
		t.Fatalf("ResponseBodyIdleTimeout = %d, want 0", got["openai"].ResponseBodyIdleTimeout)
	}
}
