...
import { ArchiveRestore, BookmarkCheck, CheckCircle2, ChevronRight, CircleDot, Clock3, CloudCheck, CloudOff, Network, Server, SlidersHorizontal, Tag as TagIcon, Tags, UserRound, Zap } from 'lucide-react'
...
function DesktopLogSkeleton({ t, showUsage, showBackup, identityAuditEnabled }: { t: (key: string) => string; showUsage: boolean; showBackup: boolean; identityAuditEnabled: boolean }) {
    return (
                ...
                <TableBody>
                    {Array.from({ length: 8 }).map((_, rowIndex) => (
                        <TableRow key={rowIndex}>
                            {Array.from({ length: 7 + Number(showUsage) + Number(showBackup) + Number(identityAuditEnabled) }).map((_, cellIndex) => (
...
export function LogTable({ logs, loading, onSelect, selectedId, identityAuditEnabled }: LogTableProps) {
    ...
    if (loading) {
        return (
            <>
                <MobileLogSkeleton />
                <DesktopLogSkeleton t={t} showUsage={showUsage} showBackup={archiveEnabled} identityAuditEnabled={identityAuditEnabled} />
            </>
        )
    }
